...
RMD_equalsize<-function(v,p,i,D){
  ...
  setMethod("show", "stat_test", function(object) {
    ...
    cat(row, "\n")
    if(D==1)cat("Following are required sets of shifts to obtain the MCPBRMD-O1 for v=ip+3", "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==2)cat("Following are required sets of shifts to obtain the MCSPBRMD-O1 for  v=ip+2" , "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==3)cat("Following are required sets of shifts to obtain the MCPBRMD-O2 for v=ip+4", "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==4)cat("Following are required sets of shifts to obtain the MCSPBRMD-O2 for v=ip+3",  "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==5)cat("Following are required sets of shifts to obtain the MCWBRMD-O1 for v=ip-1", "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==6)cat("Following are required sets of shifts to obtain the MCSGBRMD-O1 for v=ip-2", "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==7)cat("Following are required sets of shifts to obtain the MCWBRMD-O2 for v=ip-2", "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==8)cat("Following are required sets of shifts to obtain the MCSBGRMD-O2 for v=ip-3",  "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==9)cat("Following are required sets of shifts to obtain the MCPBRMD-II for v=ip+4", "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==10)cat("Following are required sets of shifts to obtain the MCSPBRMD-II for v=ip+3",  "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==11)cat("Following are required sets of shifts to obtain the MCWBRMD-II for v=ip-2 ",  "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
    if(D==12)cat("Following are required sets of shifts to obtain the MCSBGRMD-II for v=ip-3", "v=" ,object[[3]][1], "and","P=",object[[3]][2], "\n")
...
Create_Designs<-function(v,p,i){
  if(v%%2!=0){
    ...
    if((v-3)%%(p)==0 & (i=(v-3)/(p))&(i%%1==0)&(i>0) ){
      cat("MCPBND-O is possible for","v=",v,"p=",p,"i=",i,"D=1",rep("", 30))
    }
    if((v-4)%%(p)==0 & (i=(v-3)/(p))&(i%%1==0)&(i>0) ){
      cat("MCPBND-O is possible for","v=",v,"p=",p,"i=",i,"D=3",rep("", 30))
    }
    if((v-2)%%(p)==0 & (i=(v-2)/(p)) &(i%%1==0) & (i>0)) {
      cat("MCSPBRMD-O is possible for","v=",v,"p=",p,"i=",i,"D=2",rep("", 30))
    }
    if((v-3)%%(p)==0 & (i=(v-3)/(p)) &(i%%1==0) & (i>0)) {
      cat("MCSPBRMD-O is possible for","v=",v,"p=",p,"i=",i,"D=4",rep("", 30))
    }
    if((v+1)%%(p)==0 & (i=(v+1)/(p)) &(i%%1==0) & (i>0)) {
      cat("MCWBRMD-O is possible for","v=",v,"p=",p,"i=",i,"D=5",rep("", 30))
    }
    if((v+2)%%(p)==0 & (i=(v+2)/(p)) &(i%%1==0) & (i>0)) {
      cat("MCWBRMD-O is possible for","v=",v,"p=",p,"i=",i,"D=7",rep("", 30))
    }
    if((v+2)%%(p)==0 & (i=(v+2)/(p)) &(i%%1==0) & (i>0)) {
      cat("MCSBGRMD-O is possible for","v=",v,"p=",p,"i=",i,"D=6",rep("", 30))
    }
    if((v+3)%%(p)==0 & (i=(v+3)/(p)) &(i%%1==0) & (i>0)) {
      cat("MCSBGRMD-O is possible for","v=",v,"p=",p,"i=",i,"D=8",rep("", 30))
    }
    if((i%%1<0) |(i%%1>0)) {cat("Among the designs (D1 to D12) are not possible for given value ","v=",v,",", "p=",p,"and","i=",i,rep("", 30))
    }
  }else
    if(v%%2==0){
      
      if((v-4)%%(p)==0 & (i=(v-4)/(p))&(i%%1==0)&(i>0)){
        cat("MCPBRMDs-II is possible for","v=",v,"p=",p,"i=",i,"D=9",rep("",15))
        
      }
      
      if((v-3)%%(p)==0 & (i=(v-3)/(p))&(i%%1==0)&(i>0) ){
        cat(" MCSPRMDs-II is possible for","v=",v,"p=",p,"i=",i,"D=10",rep("",15))
      }
      
      if((v+3)%%(p)==0 & (i=(v+3)/(p)) & (i%%1==0) & (i>0)){
        cat("MCSBGNDs-II is possible for","v=",v,"p=",p,"i=",i,"D=12",rep("",15))
      }
      
      if((v+2)%%(p)==0 & (i=(v+2)/(p)) &(i%%1==0)&(i>0)){
        cat("MCWBNDs-II  is possible for","v=",v,"p=",p,"i=","D=11",rep("",15))
      }
      if((i%%1<0) |(i%%1>0)){cat("Among the designs (D1 to D12) are not possible for given  value","v=",v,",", "p=",p,"and","i=",i,rep("",15))
        
      }  }  }  
design_CBND<-function(H){
  
  setClass( "RMD_equalsize", representation("list"))
...
H<-Create_Designs(v=21,p=7)
# example#1
(H<-RMD_equalsize(v=23,p=7,i=3,D=2))
